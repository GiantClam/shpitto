...
- If the user did not request a specific article/resource count, render exactly 3 fallback cards and matching detail links. Do not expand a simple personal Blog into 5-6 inferred articles just because several topic tokens appear in the prompt.
...
- Do not use whole-site route choreography such as start from home, where to start, next step, homepage job, 从首页开始, 接下来看博客, 循序进入深内容, 阅读入口, 站点入口, 首页路径, 继续了解, or 下一步 when it explains browsing order instead of a concrete CTA.
