...
export class Bundler {
  static async createBundle(config: any): Promise<BundleResult> {
    ...
    const siteName = branding.name || "ShipItTo Site";
    ...
    for (const page of pages) {
      ...
      const pageDesc = page.seo?.description || "A professional website generated by ShipItTo.";
                        ...
                        <div class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all group border border-slate-100">
                            ...
                            <div class="p-8">
                                ...
                                <div class="flex items-center justify-between">
                                    ${item.price ? `<span class="font-bold text-lg text-slate-900">${item.price}</span>` : "<span></span>"}
                                    <a href="${item.link || '#'}" class="text-primary font-semibold hover:underline flex items-center gap-1">
                                        Learn more
            ...
            <div class="container mx-auto px-4">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div class="${isRight ? 'lg:order-1' : ''}">
                        ...
                        ${props.cta_text ? `
                        <a href="${props.cta_link || '#'}" class="inline-flex items-center justify-center px-8 py-4 bg-primary text-white rounded-full font-bold shadow-lg shadow-primary/25 hover:opacity-90 transition-all">
                            ${props.cta_text}
                        </a>
                        ` : ""}
                    </div>
      ...
      case "CTA_Section":
        ...
        <section class="py-24 px-4 ${ctaTheme}">
            <div class="container mx-auto max-w-5xl text-center">
                ...
                <a href="${props.cta_link || '#'}" class="inline-block px-10 py-5 ${btnTheme} rounded-full font-bold text-lg shadow-xl hover:scale-105 transition-transform">
                    ${props.cta_text}
                </a>
            </div>
        </section>`;
        ...
        <section class="py-24 bg-slate-50">
            <div class="container mx-auto px-4">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div>
                        <h2 class="text-3xl md:text-5xl font-bold mb-6">${props.title}</h2>
                        <p class="text-lg text-slate-600 mb-10">${props.description}</p>
                        ...
                        <button class="bg-primary text-white px-8 py-4 rounded-full font-bold shadow-lg shadow-primary/25 hover:opacity-90 transition-all">Explore Product</button>
                    </div>
                    <div class="relative">
                        ${props.image ? `<img src="${props.image}" class="rounded-3xl shadow-2xl relative z-10" alt="${props.title}">` : `<div class="aspect-square bg-slate-200 rounded-3xl animate-pulse"></div>`}
                    </div>
                </div>
            </div>
        </section>`;
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ${(props.items || []).map((item: any) => `
                        <div class="p-8 rounded-2xl bg-slate-50 border border-slate-100">
                            <p class="text-lg text-slate-700 italic mb-8">"${item.quote}"</p>
                            <div class="flex items-center gap-4">
                                ${item.avatar ? `<img src="${item.avatar}" class="w-12 h-12 rounded-full" alt="${item.author}">` : `<div class="w-12 h-12 bg-slate-200 rounded-full"></div>`}
                                <div>
                                    <div class="font-bold">${item.author}</div>
                                    <div class="text-sm text-slate-500">${item.position}</div>
                                </div>
                            </div>
                        </div>
                    `).join("")}
