...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (const raw of argv) {
    ...
    if (arg.startsWith("--pattern=")) {
      const value = arg.split("=", 2)[1];
      if (value) options.patterns.push(value.trim().toLowerCase());
      continue;
    }
  }
  return options;
...
async function main() {
  ...
  if (opts.patterns.length > 0) {
    process.env.CLOUDFLARE_WA_CLEANUP_HOST_PATTERNS = opts.patterns.join(",");
  }
    ...
    JSON.stringify(
      {
        ...
        patterns: opts.patterns.length > 0 ? opts.patterns : process.env.CLOUDFLARE_WA_CLEANUP_HOST_PATTERNS,
